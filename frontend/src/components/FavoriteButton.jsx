...
function FavoriteButton({ text, language }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || t('fav.err.failed'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="relative">
      <button
        onClick={handleClick}
        disabled={!text.trim()}
        title={isLoggedIn ? t('fav.tooltip') : t('fav.tooltipGuest')}
      ...
      {showForm && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-xl shadow-lg border border-gray-200 p-4 w-72">
            <h3 className="text-sm font-semibold text-gray-800 mb-3">{t('fav.title')}</h3>
            ...
            <select
              ...
              className="input-base text-sm mb-3"
            >
              <option value="">{t('fav.noGroup')}</option>
              {groups.map((g) => (
